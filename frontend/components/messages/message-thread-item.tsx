  ...
  return (
    <div className="relative">
      ...
      {thread.unreadCount && !isActive ? (
        <span
          className={`absolute top-1/2 flex h-5 min-w-5 -translate-y-1/2 items-center justify-center rounded-full bg-[var(--accent)] px-1.5 text-[10px] font-semibold text-white transition ${
            showMenuButton ? "right-3" : "right-4"
          } ${
            isMenuOpen && showMenuButton ? "opacity-0" : "opacity-100 group-hover:opacity-0 group-focus-within:opacity-0"
          }`}
        >
          {thread.unreadCount}
        </span>
