...
function parseDateParam(value: string | null, fallback = getCurrentDateLocal()) {
...
function DailyStatementsPageContent() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const defaultDate = getCurrentDateLocal();
  const dateFrom = parseDateParam(searchParams.get("date_from") ?? searchParams.get("date"), defaultDate);
  const dateTo = parseDateParam(searchParams.get("date_to") ?? searchParams.get("date"), dateFrom);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const normalizedDateFrom = parseDateParam(filterDraft.dateFrom, defaultDate);
    ...
    <main className="shell px-4 py-6 sm:px-6">
      <section className="glass-card mx-auto flex min-h-[124px] w-full max-w-[1328px] flex-col justify-center rounded-[1.5rem] px-6 py-6 sm:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-[#1f2440]">Daily Statements</h1>
      ...
      {error ? (
        <section className="glass-card mx-auto mt-5 w-full max-w-[1328px] rounded-[1.5rem] px-6 py-8 text-sm text-rose-600 sm:px-8">
          Unable to load the daily statement right now.
        </section>
      ) : null}
      ...
      {!error ? (
        <>
          ...
          <section className="glass-card mx-auto mt-5 w-full max-w-[1328px] overflow-hidden rounded-[1.5rem]">
            ...
            {isLoading ? (
              <BoneyardSkeleton name="daily-statement-products-table" loading={isLoading} fallback={<TableSkeleton columns={5} rows={4} />}>
                <TableSkeleton columns={5} rows={4} />
              </BoneyardSkeleton>
            ) : productSummaries.length === 0 ? (
              <div className="px-6 py-8 text-sm text-[var(--muted)] sm:px-8">No submitted invoice items found for this date.</div>
            ) : (
          ...
          <section className="glass-card mx-auto mt-5 w-full max-w-[1328px] overflow-hidden rounded-[1.5rem]">
            <div className="border-b border-[var(--line)] px-6 py-5 sm:px-8">
              <h2 className="text-xl font-semibold text-[#1f2440]">Invoices</h2>
            </div>
            {isLoading ? (
              <BoneyardSkeleton name="daily-statement-invoices-table" loading={isLoading} fallback={<TableSkeleton columns={6} rows={5} />}>
                <TableSkeleton columns={6} rows={5} />
              </BoneyardSkeleton>
            ) : statementInvoices.length === 0 ? (
              <div className="px-6 py-8 text-sm text-[var(--muted)] sm:px-8">No submitted invoices found for this date.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-[var(--line)] text-left text-xs uppercase tracking-[0.18em] text-[var(--muted)]">
                      <th className="px-6 py-4 font-semibold sm:px-8">Invoice</th>
                  ...
                  <tbody>
                    {statementInvoices.map((invoice) => (
                      <tr key={invoice.id} className="border-b border-[var(--line)] text-sm last:border-0">
                        <td className="px-6 py-4 font-medium text-[#2d3150] sm:px-8">{invoice.invoice_no}</td>
...
function DailyStatementsPageFallback() {
  return (
    <main className="shell px-4 py-6 sm:px-6">
      <section className="glass-card mx-auto w-full max-w-[1328px] rounded-[1.5rem]">
        <BoneyardSkeleton name="daily-statement-page-fallback" loading fallback={<PanelSkeleton lines={5} />}>
...
export default function DailyStatementsPage() {
  return (
    <Suspense fallback={<DailyStatementsPageFallback />}>
      <DailyStatementsPageContent />
    </Suspense>
  );
}
